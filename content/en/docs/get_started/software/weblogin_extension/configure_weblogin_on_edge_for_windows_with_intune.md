---
title: "Configure on Edge for Windows using Intune"
description: "How to configure the IDmelon WebLogin extension on Edge with Intune."
lead: ""
...
This guide provides a PowerShell script for IT Administrators to configure the WebLogin extension on Microsoft Edge via
...
$registry_path = "HKLM:\SOFTWARE\Policies\Microsoft\Edge\3rdparty\extensions\eagmgpbjpedchliifpgfgogdknnmkaej\policy"
...
4. **Basics:** Name the script (e.g., Edge - WebLogin Extension Managed Config).
...
5. **Script Settings**:
   ...
   ![alt](/images/vendor/weblogin/script_settings_edge.jpg)
...
1. **Check the Registry:** Open `regedit` and navigate to:
   `HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Edge\3rdparty\extensions\eagmgpbjpedchliifpgfgogdknnmkaej\policy`
   Verify that all declared keys (`apiKey`, `serverAddress`, etc.) are present.

2. **Verify in Edge:** Open Edge and navigate to `edge://policy`. Find the entry for
